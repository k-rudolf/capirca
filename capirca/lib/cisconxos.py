...
class TermStandard(object):
  ...
  def __init__(self, term, filter_name, platform='cisconxos', verbose=True):
    self.term = term
...
class PortMap(object):
    ...
    @staticmethod
    def getProtocol(port_num, proto, platform='cisconxos'):
...
class Term(aclgenerator.Term):
  ...
  def __init__(self, term, af=4, proto_int=True, enable_dsmo=False,
               term_remark=True, platform='cisconxos', verbose=True):
  ...
  def __str__(self):
    ...
    if self.term.source_address:
      ...
      #print(source_address) -> IP address object
...
class CiscoNxOS(aclgenerator.ACLGenerator):
  #print("BLA")
  ...
  def _BuildTokens(self):
    ...
    """
    supported_tokens, supported_sub_tokens = super(CiscoNxOS, self)._BuildTokens()
      ...
      for filter_type in filter_list:
        ...
        if self.verbose:
          ...
          for comment in header.comment:
            for line in comment.split('\n'):
              if (self._PLATFORM == 'cisconxos' and filter_type == 'standard' and
                  filter_name.isdigit()):
